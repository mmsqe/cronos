...
def test_grpc_mode(custom_cronos):
    """
    ...
    assert 777 == contract.caller.currentChainID()

    msg = {
        "to": contract.address,
        "data": contract.encodeABI(fn_name="currentChainID"),
    }
    api_port = ports.api_port(custom_cronos.base_port(1))
    ...
    for i in range(max_retry):
        ...
        if valid and 777 == int.from_bytes(base64.b64decode(ret.encode()), "big"):
            success = True
            break
        time.sleep(sleep)
    assert success
    ...
    for i in range(2):
        wait_for_block(custom_cronos.cosmos_cli(i), 1)
    supervisorctl(
        custom_cronos.base_dir / "../tasks.ini", "stop", "cronos_777-1-node1"
    )

    # run grpc-only mode directly with existing chain state
    with (custom_cronos.base_dir / "node1.log").open("a") as logfile:
        ...
        try:
            ...
            grpc_port = ports.grpc_port(custom_cronos.base_port(1))
